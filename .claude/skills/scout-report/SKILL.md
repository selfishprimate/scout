---
name: scout-report
description: Summarise the job search from the markdown tracker - funnel, response rate, what's working, where applications come from, what's pending on the user. Use when the user runs /scout-report or asks how the search is going, for weekly numbers, or what to change.
---

# /scout-report — how is the search going

Read-only. Data comes from `applications/` via `scripts/scout.py`, plus `runs/*.md` for per-day source tables.

1. `python3 scripts/scout.py stats` (all time) and `python3 scripts/scout.py stats --since <7 days ago>`.
2. `python3 scripts/scout.py list --since <period start>`, then read the front matter of the rows you need (source, ats, location_fit, fit).
