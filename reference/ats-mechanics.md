...
- **File source:** `file_upload` reads only `/mnt/user-data/uploads/Career/` (not Drive, not `/mnt/user-data/outputs/`). Scheduled runs lack that mount → CV forms need a live session. Claude's own browser panel (`mcp__remote-devices__Claude_Browser__*`) has no upload → CV forms need Chrome (`mcp__claude-in-chrome__*`). Page-side CV fetch (CSP/CORS) and base64 injection don't work.
